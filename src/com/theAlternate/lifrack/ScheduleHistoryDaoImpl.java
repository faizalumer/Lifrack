...
public class ScheduleHistoryDaoImpl implements ScheduleHistoryDao{
	...
	public void updateInvalidatedTime(Schedule schedule){
		if(!schedule.hasValidId()){
			throw new IllegalStateException("This schedule object does not have a valid ID. Cannot update.");
		}
		
		ContentValues values = new ContentValues();
		values.put(Table_HabitSchedules.COLUMN_INVALIDATED_TIME, Utilities.getSimpleDateFormatUTC().format(schedule.getInvalidatedTime()));
